...
<Grid item xs={12}>
  ...
  {/* NGO ID Filter (Single Selection) */}
{/* NGO ID Filter (Single Selection) */}
<Grid item xs={12} sm={6}>
  <FormControl fullWidth>
    <InputLabel>NGO</InputLabel>
    <Select
      value={ngoId || ""} // ✅ Default to empty for "All"
      onChange={(e) => setNgoId(e.target.value === "all" ? null : e.target.value)}
      renderValue={(selected) =>
        selected
          ? ngos.find((ngo) => ngo.universal_user_id === selected)?.ngo_name || ""
          : "All NGOs"
      }
    >
      {/* ✅ Option to Remove NGO Filter */}
          ...
          <TableBody>
            {sales.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell sx={{ color: colors.text, fontWeight: "bold" }}>{item.ngo_name || "N/A"}</TableCell>
                <TableCell sx={{ color: colors.text }}>{item.category_name || "N/A"}</TableCell> {/* ✅ Added Category */}
            ...
            {/* ✅ Grand Total Row */}
            <TableRow>
              <TableCell colSpan={4} sx={{ color: colors.text, fontWeight: "bold", textAlign: "right" }}>
                Total NGO Sales:
              </TableCell>
