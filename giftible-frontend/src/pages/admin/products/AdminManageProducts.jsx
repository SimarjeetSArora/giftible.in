...
const AdminManageProducts = () => {
  ...
  const [ngoId, setNgoId] = useState([]);  // ✅ Store multiple NGO IDs
  const [categoryId, setCategoryId] = useState([]);  // ✅ Store multiple Category IDs
  ...
  const downloadPDF = () => {
    ...
    // ✅ Table Headers
    const tableColumn = [
      "ID", "Product Name", "Category", "NGO", "Price", "Stock", "Status", "Created At"
    ];
    const tableRows = [];
...
// ✅ Only add `ngo_ids` and `category_ids` if selected
if (ngoId.length > 0) params.ngo_ids = ngoId;
...
<FormControl fullWidth>
  ...
  <Select
    ...
    renderValue={(selected) =>
      selected.map((id) => ngos.find((ngo) => ngo.universal_user_id === id)?.ngo_name).join(", ")
    }
  >
    {ngos.map((ngo) => (
      <MenuItem key={ngo.universal_user_id} value={ngo.universal_user_id}>
        <Checkbox checked={ngoId.includes(ngo.universal_user_id)} /> {/* ✅ Checkbox inside dropdown */}
        {ngo.ngo_name}
      </MenuItem>
    ))}
  </Select>
  ...
  >
    {categories.map((category) => (
      <MenuItem key={category.id} value={category.id}>
        <Checkbox checked={categoryId.includes(category.id)} /> {/* ✅ Checkbox inside dropdown */}
        {category.name}
      </MenuItem>
    ))}
  </Select>
